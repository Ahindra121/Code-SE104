...
def my_enrollments(db: Session = Depends(get_db), current_user: User = Depends(require_roles(UserRole.student))):
    rows = db.scalars(
        select(Enrollment).options(joinedload(Enrollment.course)).where(Enrollment.student_id == current_user.id)
    ).unique().all()
    return ok([EnrollmentOut.model_validate(row) for row in rows])
